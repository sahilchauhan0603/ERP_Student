...
import { 
  FiX, FiCheck, FiXCircle, FiFileText, FiUser, FiBook, FiHome, 
  FiDownload, FiArrowRight, FiArrowLeft, FiCheckCircle, FiAlertCircle,
  FiAward, FiDollarSign, FiUsers, FiCalendar
} from 'react-icons/fi';
...
export default function AdminStudentDetailsModal({ student, onClose, refresh, tableType }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`${import.meta.env.VITE_API_URL}/admin/student-details/${student.id}`)
  ...
  const isSectionVerified = (sectionKey) => {
    const sectionFields = {
      personal: ['firstName', 'middleName', 'lastName', 'email', 'mobile', 'dob', 'gender', 'category', 'subCategory', 'region', 'currentAddress', 'permanentAddress', 'course', 'examRoll', 'examRank', 'abcId'],
      academic: ['classX_institute', 'classX_board', 'classX_year', 'classX_aggregate', 'classX_pcm', 'classX_isDiplomaOrPolytechnic', 'classXII_institute', 'classXII_board', 'classXII_year', 'classXII_aggregate', 'classXII_pcm', 'otherQualification_institute', 'otherQualification_board', 'otherQualification_year', 'otherQualification_aggregate', 'otherQualification_pcm', 'academicAchievements', 'coCurricularAchievements'],
      parent: ['father_name', 'father_mobile', 'father_email', 'mother_name', 'mother_mobile', 'mother_email', 'family_income'],
      documents: ['photo', 'ipuRegistration', 'allotmentLetter', 'examAdmitCard', 'examScoreCard', 'marksheet10', 'passing10', 'marksheet12', 'passing12', 'aadhar', 'characterCertificate', 'medicalCertificate', 'migrationCertificate', 'categoryCertificate', 'specialCategoryCertificate', 'academicFeeReceipt', 'collegeFeeReceipt', 'parentSignature']
    };
    return sectionFields[sectionKey].every(f => verifications[sectionKey]?.[f] !== undefined);
  };
  ...
  setVerifications(prev => ({
    ...prev,
    [section]: sectionFields[section].reduce((acc, field) => ({
      ...acc,
  ...
  const handleProceed = () => {
    setSectionCompleted(prev => [...prev.map((v, i) => i === activeTab ? true : v)]);
...
const DetailField = ({ label, value, section, field, isDocument, showVerify }) => {
    ...
    if ((field === 'academicAchievements' || field === 'coCurricularAchievements') && value) {
      ...
      if (Array.isArray(displayValue)) {
        displayValue = displayValue.length === 0 ? <span className="text-gray-400">N/A</span> : (
          <ul className="list-disc pl-5 space-y-1">
            {displayValue.map((item, idx) => (
              <li key={idx} className="text-sm text-gray-700">
                {item.event && <span className="font-medium">{item.event}</span>}
                {item.date && <span className="ml-2 text-gray-500">({item.date})</span>}
                {item.outcome && <span className="ml-2 text-blue-600">- {item.outcome}</span>}
              </li>
            ))}
          </ul>
        );
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            ...
            <div className="mt-1 text-gray-600 text-sm">
              {isDocument && value ? (
                <a href={value} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 flex items-center">
          ...
          {showVerify && (
            <div className="flex space-x-2 ml-4">
              <button
                ...
                className={`p-1.5 rounded-full ${verifiedStatus === true ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400 hover:bg-green-50'}`}
              ...
              <button
                ...
                className={`p-1.5 rounded-full ${verifiedStatus === false ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-400 hover:bg-red-50'}`}
  ...
  const renderDocuments = () => {
    const docFields = [
      ...
      { label: 'Special Category Certificate', key: 'specialCategoryCertificate' },
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="px-6 pt-4">
              ...
              {sectionDefs.map((section, index) => (
                <div key={index} className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    sectionCompleted[index] ? 'bg-green-100 text-green-600' :
                    activeTab === index ? 'bg-blue-100 text-blue-600' :
                    'bg-gray-100 text-gray-400'
                  }`}>
                    {sectionCompleted[index] ? <FiCheckCircle /> : index + 1}
                  </div>
                  <span className={`text-xs mt-1 ${
                    activeTab === index ? 'font-medium text-blue-600' : 'text-gray-500'
                  }`}>
                    {section.label}
                  </span>
                </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div 
                ...
                style={{ width: `${((activeTab + (sectionCompleted[activeTab] ? 1 : 0)) / sectionDefs.length) * 100}%` }}
              ></div>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          {loading ? (
            ...
            <div className="text-center py-10 text-red-500">Failed to load student details</div>
          ) : (
